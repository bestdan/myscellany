---
title: "Forward-testing strategies"
author: "Daniel Egan"
date: "11/12/2017"
output: 
  html_document: 
...
pacman::p_load(ggplot2, scales, dplyr, knitr, janitor, ggridges, here, reshape2, gridExtra)
...
This RMD supports the post "Forward testing"
...
ggplot() +
  ...
  geom_line(data = market_sim_returns_alternative_cumul %>% filter(Var2==4), 
            aes(x=Var1, y=value), color = "dark green")  + 
  ylab("Growth of $1") + xlab("Years") + 
...
> If the trailing `r window` day returns are above `r percent(threshold_annual)` annualized, we assume we're in a "good" market, and invest. If they are beneath `r percent(threshold_annual)`, we assume we're in a "bad" market, and don't invest. We earn no return when not invested. 
...
ggplot(cumul_values_all_wide_alternative %>% filter(day %in% round(seq(252, nobs, 252)))) +
  ...
  geom_vline(xintercept = 0, col="dark grey") + scale_x_continuous(label=percent) + 
  scale_y_discrete(labels = dayToYear) + ggtitle("Active outperformance by years")+
  ylab("Years") + xlab("Active outperformance") + coord_flip()

```


```{r}
plot1 <- ggplot(cumul_values_all_wide_alternative %>% filter(day %in% round(seq(252, nobs, 252)))) +
  ...
  scale_y_discrete(labels = dayToYear) + ggtitle("Passive Final Value") + 
  ylab("Years") + xlab("Baseline raw performance") + coord_flip()

plot2 <- ggplot(cumul_values_all_wide_alternative %>% filter(day %in% round(seq(252, nobs, 252)))) +
  geom_density_ridges2(aes(x=value.active, y=as.factor(day), scale=1), fill=light_blue_trans, color = NA) + 
  geom_vline(xintercept = 1, col="dark grey") + scale_x_continuous(label=percent) + 
  scale_y_discrete(labels = dayToYear) + ggtitle("Active Final Value") + 
  ylab("Years") + xlab("Baseline raw performance") + coord_flip()
...
win_ratio_string <- filter(win_ratio, day %in% c(c(0, 30, 90, 120, 180, 240), seq(252, 252 * 5, 252))) 
win_ratio_string$win_rate <- percent(win_ratio_string$win_rate)

kable(win_ratio_string)
...
ggplot(filter(win_ratio, day <= 252 * 5)) +
  ...
  xlab("Years") + ylab("Strategy Win Ratio") + ggtitle("Active strategy win ratio")
...
ggplot(final_values_comp) + 
  geom_density(aes(x=value, group = strategy, fill=strategy), alpha = 0.5, color=NA) + 
  ggtitle("Final value by active vs baseline") + scale_x_continuous(labels = dollar)
```
...
_It's funny, through the course of writing this document up, I could feel myself getting hopeful that I'd beat a market-beating strategy... even though I knew I'd set it up to tell me that!! **Beware confirmation bias**_
