...
window <- 22
threshold_annual <- 0.02 # This parameter controls when the alternatie stratey believes it's in a 'good' or 'bad' market.
threshold_daily <- ((1 + threshold_annual) ^ (1/252)) - 1  # Convert to daily
set.seed(8675309)
...
processStrategy <- function(return_matrix, strategy){
  if(strategy == "baseline"){
    ...
    signals[which(mean_returns >= threshold_daily)] <- 1
    signals[which(mean_returns < threshold_daily)] <- 0
    signals[1:window,] <- 1  # Auto-invest in burn-in period.
    signals <- rbind(rep(1, ncol(signals)), tail(signals, -1))  # Offset so that the decision is implemented on T+1
