...
cumulative_results <- function(risk_level,
                               ptiles = c(0.20, 0.5, 0.80),
                               years = 10,
                               nobs = 100000) {
  ps <- portstats(risk_level)
  # Generate a matrix of returns
  raw_returns <-
    matrix(rnorm(n = nobs, mean = ps['er'], sd = ps['vol']) / 100,
           nrow = years)
  returns_cumul <-
    apply(raw_returns, 2, function(x) {
      cumprod(x + 1)
    }) - 1
  
  # For each year, find the pdf of cumulative returns
  returns_cumul_dens_by_year <- apply(returns_cumul, 1, density)
  names(returns_cumul_dens_by_year) <-
    paste0("year_", seq_len(length(returns_cumul_dens_by_year)))
  
  # Create a list of data.frames the PDFs for each year.
  cumul_densities <-
    lapply(seq_along(returns_cumul_dens_by_year), function(i) {
      z <- returns_cumul_dens_by_year[[i]]
      data.frame(
        year = as.numeric(gsub(
          "year_", "", names(returns_cumul_dens_by_year)[[i]]
        )),
        risk = risk_level,
        loc = z$x,
        dens = z$y
  ...
  cumul_quantiles <-
    melt(cumul_quantiles, id = "year", variable.name = "percentile")
  cumul_quantiles$percentile <-
    as.numeric(gsub("[^0-9\\]", "", cumul_quantiles$percentile)) / 100
...
ggplot_vertical_dist <- function(df,
                                 åågroup_var,
                                 ylims = NULL,
                                 quantile_data = NULL) {
  ...
  if (nlevels(df[, group_var]) < 2)
    warning("group_var only has 1 level.")
  
  tp <-  ggplot(data = df, aes_string(y = group_var,
                                      x = "loc",
                                      height = "dens"))
  
  tp <- tp + geom_density_ridges(
    scale = 0.9,
    stat = "identity",
  ...
  if (!is.null(quantile_data)) {
    ...
    tp <-
      tp + geom_point(
        data = quantile_data,
        aes_string(x = "value", y = group_var),
        size = 2,
        color = "red"
      )
  }
