...
one_year_outcomes_df <- data.frame()
for(risk_level in one_year_risk_levels){
  ps <- portstats(risk_level)
  temp_dist <- rnorm(n = 10000, mean = ps['er'], sd = ps['vol']) / 100
  one_year_outcomes_df <- rbind(one_year_outcomes_df, 
                                data.frame(risk_level = risk_level, value=temp_dist))
}
head(one_year_outcomes_df)
one_year_medians <- aggregate(value ~ risk_level, data =one_year_outcomes_df, FUN = median)
...
```{r}
pdat <- outcomeDensity(one_year_outcomes_df, group_var = "risk_level")
ggplot_vertical_dist(pdat, group_var="risk_level")
